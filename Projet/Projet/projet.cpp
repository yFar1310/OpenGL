...
Point pico[12]={
   {milieu(pCube[0],pCube[1],0,0,0)},//m0
   {milieu(pCube[1],pCube[2],1,0,0)},//m1
   {milieu(pCube[2],pCube[3],1,1,0)},//m2
   {milieu(pCube[0],pCube[3],1,1,1)},//m3
   {milieu(pCube[0],pCube[4],1,1,1)},//m4
   {milieu(pCube[1],pCube[5],0,1,1)},//m5
   {milieu(pCube[2],pCube[6],0,0,1)},//m6
   {milieu(pCube[3],pCube[7],0,0,0)},//m7
   {milieu(pCube[4],pCube[5],1,0,0)},//m8
   {milieu(pCube[5],pCube[6],1,1,0)},//m9
   {milieu(pCube[6],pCube[7],1,0,1)},//m10
...
double control=1;
Point Sico[12]={
   {sommetIco(pCube[0],pCube[1],0,1,0,control)},
   {sommetIco(pCube[1],pCube[2],0,1,0,control)},
   {sommetIco(pCube[2],pCube[3],0,1,0,control)},
   {sommetIco(pCube[3],pCube[0],0,1,0,control)},
   {sommetIco(pCube[0],pCube[4],0,1,0,control)},
   {sommetIco(pCube[1],pCube[5],0,1,0,control)},
   {sommetIco(pCube[2],pCube[6],0,1,0,control)},
   {sommetIco(pCube[3],pCube[7],0,1,0,control)},
   {sommetIco(pCube[4],pCube[5],0,1,0,control)},
   {sommetIco(pCube[5],pCube[6],0,1,0,control)},
   {sommetIco(pCube[6],pCube[7],0,1,0,control)},
   {sommetIco(pCube[7],pCube[4],0,1,0,control)},
...
void Ellipsoïde(float a, float b,float c, int n);
   ...
   glBegin(GL_QUADS);
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            float x1 = a * cos(t1) * sin(t2);
            float y1 = b * sin(t1) * sin(t2);
            float z1 = c * cos(t2);

            float x2 = a * cos(t1_next) * sin(t2);
            float y2 = b * sin(t1_next) * sin(t2);
            float z2 = c * cos(t2);

            float x3 = a * cos(t1_next) * sin(t2_next);
            float y3 = b * sin(t1_next) * sin(t2_next);
            float z3 = c * cos(t2_next);

            float x4 = a * cos(t1) * sin(t2_next);
            float y4 = b * sin(t1) * sin(t2_next);
            float z4 = c * cos(t2_next);
...
void affichage()
{
   ...
   */
   glPushMatrix();
    glTranslatef(0.5+0.5/2,1,0);
    NotreIcosaedre();
   glPopMatrix();

   glPushMatrix();
      glColor3f(1.0,0.0,0.0);                  //SPHÈRE
      glTranslatef(1,1.25,0);
     glutSolidSphere(0.5,16,16);
   glPopMatrix();
   /*glPushMatrix();
    glTranslatef(0.5+0.5/2,0.5,0);
    NotreIcosaedre();
   glPopMatrix();
   glPushMatrix();
   ...
   glPushMatrix();
   glTranslatef(1,1.25+0.75,0.2);
   Ellipsoïde(0.1,0.5,0.1,30);
   glPopMatrix();
   glPushMatrix();
   glTranslatef(1,1.25+0.75,-0.2);
   Ellipsoïde(0.1,0.5,0.1,30);
   ...
   glPushMatrix();
     ...
     gluCylinder(quad,(0.2),(0.25),0.25,16,1);
   glPopMatrix();
   glPushMatrix();
     glTranslatef(0,0,0);
     glRotatef(-90,1,0,0);
     glRotatef(-90,0,1,0);
     glTranslatef(1,0,-0.5);//moitié bien calculé
     glutSolidCone(0.3,0.8,5,6);
   glPopMatrix();
